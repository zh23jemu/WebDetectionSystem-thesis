...
YOLO（You Only Look Once）属于典型的单阶段目标检测算法，其主要特点是在一次前向推理过程中完成目标定位和类别判断。本系统使用 Ultralytics 提供的 YOLO 接口加载训练好的 `best.pt` 权重文件，对上传或摄像头采集的木质板材图片进行检测，得到缺陷类别、置信度和带检测框的结果图。

在系统应用中，后端检测函数会先读取管理员设置的置信度阈值和当前模型文件，再调用 YOLO 模型完成推理。模型返回结果后，系统将英文类别名称转换为中文缺陷名称，并按照缺陷严重程度生成板材等级。例如，无明显缺陷时判定为 A 级，检测到活节时判定为 B 级，检测到裂纹或死节时判定为 C 级。这样处理后，目标检测结果可以直接服务于结果展示、历史记录保存、库存更新和销售出库等后续业务。
...
从经济角度看，本系统主要成本集中在开发时间、测试图片准备和本地运行环境调试等方面，不需要购买专门的商业软件。系统开发使用的 Python、Flask、SQLite、Ultralytics、OpenCV、Bootstrap 和 Chart.js 等工具或组件均可在普通个人计算机上完成安装和使用，数据库采用 SQLite 文件存储方式，也不需要额外部署独立数据库服务器。检测演示所需图片可以通过已有样本和系统上传功能完成准备，整体硬件和软件投入较低，符合毕业设计阶段的开发条件。

### 3.1.2 操作可行性

从操作角度看，系统采用浏览器访问方式，用户登录后可通过导航栏进入缺陷检测、历史记录、日志报表、销售管理等页面。检测页面支持本地上传和摄像头采集两种方式，后台设置集中在管理员页面中，整体操作流程较清楚，能够满足普通用户和管理员的基本使用要求。

### 3.1.3 技术可行性

从技术角度看，系统所使用的 Flask、SQLite、YOLO 模型接口和前端页面组件均较成熟，相关资料较容易获取。当前系统已经能够完成用户登录、图片检测、结果保存、库存更新和销售记录写入等主要流程，说明所选技术路线能够支撑系统核心功能的实现。
